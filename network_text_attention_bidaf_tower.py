...
def build_graph(FLAGS, vocab_embedding, sub, q, a0, a1, a2, a3, a4, a, qid, sub_seq_len, q_seq_len, a0_seq_len, a1_seq_len, a2_seq_len, a3_seq_len, a4_seq_len, prob):
    ...
    with tf.variable_scope("prediction"):
        ...
        cost = tf.reduce_mean(loss) + tf.cast(tf.contrib.layers.apply_regularization(regularizer, reg_losses),
                                              tf.float32) + (FLAGS.wd * tf.reduce_sum([tf.nn.l2_loss(v) for v in reg_var_list]))
        ...
        summary_op = tf.summary.merge([loss_summary, accuracy_summary])
        '''
    return cost, accuracy, qid, comparison
