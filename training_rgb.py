...
tf.flags.DEFINE_integer("num_epoch", 50, "number of epoch")
tf.flags.DEFINE_float('lr_i3d', 1e-2, 'i3d_learning_rate')
tf.flags.DEFINE_float('lr_match', 3e-4, 'match_learning_rate')
...
checkpoint_path = './log/checkpoint_rgb_and_text_no_se_lstm'
    ...
    def __call__(self):
        with hf.File(self.video_file, "r") as data:
            for qa in self.qa_file:
                ...
                return_value = {'rgb': rgb_images, 'sub': subtitles, 'q': question,
                                'a0': answer0, 'a1': answer1, 'a2': answer2, 'a3': answer3, 'a4': answer4,
                                'a': answer, 'qid': [qa['qid']], 'rgb_seq_len': [len(rgb_images)], 'sub_seq_len': [len(subtitles)],
...
def tower_loss(scope, vocab_embedding, next_element):
    _, accuracy, video_accuracy, qid, comparison, train_i3d_var_list, train_match_var_list = network_tower_rgb.build_graph(FLAGS, vocab_embedding,
                                                                             next_element['rgb'],
                                                                             ...
                                                                             next_element['qid'], next_element['rgb_seq_len'],
    ...
    #for l in losses:
    loss_name = re.sub("%s_[0-9]*/" % network_tower_rgb.TOWER_NAME, '', _.op.name)
    tf.summary.scalar(loss_name, _)
    return _, accuracy, video_accuracy, qid, comparison, train_i3d_var_list, train_match_var_list
...
def main(_):
    ...
    train_dataset = tf.data.Dataset.from_generator(train_generator, output_types={'rgb': tf.float32,
                                                                                  'sub': tf.int32, 'q': tf.int32,
                                                                                  'a0': tf.int32, 'a1': tf.int32,
                                                                                  'a2': tf.int32, 'a3': tf.int32,
                                                                                  'a4': tf.int32, 'a': tf.int32,
                                                                                  'qid': tf.int32, 'rgb_seq_len': tf.int32,
    ...
    train_dataset = train_dataset.padded_batch(FLAGS.batch_size,
                                               padded_shapes={'rgb': [None, 224, 224, 3],
                                                              'sub': [None], 'q': [None], 'a0': [None], 'a1': [None],
                                                              'a2': [None], 'a3': [None], 'a4': [None], 'a': [5],
                                                              'qid': [1], 'rgb_seq_len': [1], 'sub_seq_len': [1], 'q_seq_len': [1],
    ...
    val_dataset = tf.data.Dataset.from_generator(val_generator, output_types={'rgb': tf.float32,
                                                                              'sub': tf.int32, 'q': tf.int32,
                                                                              'a0': tf.int32, 'a1': tf.int32,
                                                                              'a2': tf.int32, 'a3': tf.int32,
                                                                              'a4': tf.int32, 'a': tf.int32,
    ...
    #train_dataset = train_dataset.shuffle(130000)
    ...
    with tf.device('/cpu:0'):
        ...
        i3d_decay_steps = int(num_batches_per_epoch * 5)
        match_decay_steps = int(num_batches_per_epoch * 3)
        lr_i3d = tf.train.exponential_decay(FLAGS.lr_i3d, global_step, i3d_decay_steps, 0.9, staircase=True)
        lr_match = tf.train.exponential_decay(FLAGS.lr_match, global_step, match_decay_steps, 0.9, staircase=True)
        #opt = tf.train.MomentumOptimizer(learning_rate=lr, momentum=0.9)
        opt_i3d = tf.train.MomentumOptimizer(learning_rate=lr_i3d, momentum=0.9)
        opt_match = tf.train.AdamOptimizer(learning_rate=lr_match, epsilon=0.1)
        i3d_tower_grads = []
        match_tower_grads = []
        accuracy_list = []
        video_accuracy_list = []
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            for j in range(FLAGS.num_gpus):
                ...
                with tf.device('/gpu:%d' % j):
                    with tf.name_scope('%s_%d' % (network_tower_rgb.TOWER_NAME, j)) as scope:
                        loss, accuracy, video_accuracy, qid, comparison, train_i3d_var_list, train_match_var_list = tower_loss(scope, vocab_embedding, next_element)
                        summaries.extend(tf.get_collection(tf.GraphKeys.SUMMARIES, scope))
                        i3d_grads = opt_i3d.compute_gradients(loss, var_list=train_i3d_var_list)
                        match_grads = opt_match.compute_gradients(loss, var_list=train_match_var_list)
                        i3d_tower_grads.append(i3d_grads)
                        match_tower_grads.append(match_grads)
                        tower_qid.append(qid)
                        accuracy_list.append(accuracy)
                        comparison_list.append(comparison)
                        video_accuracy_list.append(video_accuracy)

        i3d_grads = average_gradients(i3d_tower_grads)
        match_grads = average_gradients(match_tower_grads)
        ...
        summaries.append(tf.summary.scalar('i3d_learning_rate', lr_i3d))
        summaries.append(tf.summary.scalar('match_learning_rate', lr_match))
        '''
        for grad, var in grads:
            if grad is not None:
                summaries.append(tf.summary.histogram(var.op.name + '/gradients', grad))
        '''
        i3d_apply_gradient_op = opt_i3d.apply_gradients(i3d_grads)
        match_apply_gradient_op = opt_match.apply_gradients(match_grads, global_step=global_step)
        '''
        ...
        train_op = tf.group(i3d_apply_gradient_op, match_apply_gradient_op, update_ops)

        config = tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True), allow_soft_placement=True)
        rgb_variable_map = {}
        for variable in tf.global_variables():
            if variable.name.split('/')[0] == 'RGB' and not "Momentum" in variable.name and not "rgb_fc" in variable.name and not "se3d" in variable.name:
        ...
        for v in tf.global_variables():
            if v.name.startswith(u"Text") or v.name.startswith(u"post_rnn"):
        ...
        with tf.Session(config=config) as sess:
            ...
            #network_saver.restore(sess, "./log/checkpoint_rgb_and_text_se/model-228810")
            rgb_saver.restore(sess, _CHECKPOINT_PATHS['rgb_imagenet'])
            #flow_saver.restore(sess, _CHECKPOINT_PATHS['flow_imagenet'])
            text_saver.restore(sess, "./log/best_result/text1/model-37167")
            train_summary_writer = tf.summary.FileWriter('./log/train_rgb_and_text_no_se_lstm', sess.graph)
            val_summary_writer = tf.summary.FileWriter('./log/val_rgb_and_text_no_se_lstm', sess.graph)
            ...
            for i in range(FLAGS.num_epoch):
                ...
                with open("val_log.txt", 'a') as f:
                    ...
                    while True:
                        try:
                            #next_element = iterator.get_next()
                            output = sess.run({'video_accuracy': video_accuracy_avg, 'summary_op': summary_op, 'comparison': comparison_list})
                            #f.write(str(outputs['qid']) + ': ' + str(outputs[comparison]) + '\n')
                            val_summary_writer.add_summary(output['summary_op'], global_step=val_step)
                            cum_acc += output['video_accuracy']
                            comparisons = np.concatenate((output['comparison'][0], output['comparison'][1])).tolist()
                            ...
                            break
                    print "epoch: " + str(i + 1) + " val_accuracy: " + str(list_correct / float(15253))
                    print "epoch: " + str(i + 1) + " video_accuracy: " + str(cum_acc / k)
