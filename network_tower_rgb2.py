...
def build_graph(FLAGS, vocab_embedding, rgb_input, sub, q, a0, a1, a2, a3, a4, a, qid, sub_seq_len,
    ...
    with tf.variable_scope("RGB_question_answer_att_layer", reuse=tf.AUTO_REUSE):
        ...
        rgb_question_answer0_att = tf.reduce_max(rgb_question_answer0_att, 1)
        rgb_question_answer1_att = tf.reduce_max(rgb_question_answer1_att, 1)
        rgb_question_answer2_att = tf.reduce_max(rgb_question_answer2_att, 1)
        rgb_question_answer3_att = tf.reduce_max(rgb_question_answer3_att, 1)
        rgb_question_answer4_att = tf.reduce_max(rgb_question_answer4_att, 1)
    ...
    with tf.variable_scope("prediction"):
        ...
        for v in tf.trainable_variables():
            if (v.name.startswith(u"RGB") or v.name.startswith(u"video")):
