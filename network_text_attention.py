    ...
    #with tf.device("/GPU:2"):
        ...
        with tf.variable_scope("Text_GRU", reuse=tf.AUTO_REUSE):
            ...
            subtitle_context_vector = tf.get_variable(name='subtitle_context_vector', shape=[FLAGS.num_hidden * 2], trainable=True,
                                             dtype=tf.float64, regularizer=regularizer)
            ...
            subtitle_attention = tf.layers.dense(subtitle_rnn_logits, FLAGS.num_hidden * 2, activation=tf.nn.relu,
                                                  name="subtitle_attention_layer", kernel_regularizer=regularizer)
            subtitle_attention = tf.multiply(subtitle_attention, subtitle_context_vector)
            subtitle_attention = tf.nn.softmax(subtitle_attention, axis=1)
            ...
            question_attention = tf.layers.dense(question_rnn_logits, FLAGS.num_hidden * 2, activation=tf.nn.relu,
                                                  name="question_attention_layer", kernel_regularizer=regularizer)
            question_attention = tf.multiply(question_attention, question_context_vector)
            question_attention = tf.nn.softmax(question_attention, axis=1)
            ...
            answer0_attention = tf.layers.dense(answer0_rnn_logits, FLAGS.num_hidden * 2, activation=tf.nn.relu,
                                                    name="answer_attention_layer", kernel_regularizer=regularizer)
            answer1_attention = tf.layers.dense(answer1_rnn_logits, FLAGS.num_hidden * 2, activation=tf.nn.relu,
                                                    name="answer_attention_layer", reuse=True, kernel_regularizer=regularizer)
            ...
            answer0_attention = tf.nn.softmax(answer0_attention, axis=1)
            answer1_attention = tf.nn.softmax(answer1_attention, axis=1)
            answer2_attention = tf.nn.softmax(answer2_attention, axis=1)
            answer3_attention = tf.nn.softmax(answer3_attention, axis=1)
            answer4_attention = tf.nn.softmax(answer4_attention, axis=1)
