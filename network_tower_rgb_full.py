...
def build_graph(FLAGS, vocab_embedding, rgb_input, sub, q, a0, a1, a2, a3, a4, a, qid, sub_seq_len,
                q_seq_len, a0_seq_len, a1_seq_len, a2_seq_len, a3_seq_len, a4_seq_len, prob, is_training):
    ...
    with tf.variable_scope("Text_LSTM", reuse=tf.AUTO_REUSE):
        ...
        question_rnn_logits = text_rnn_model.build(q_tensor, dropout_keep_prob=prob, seq_len=q_seq_len)
        subtitle_rnn_logits = text_rnn_model.build(sub_tensor, dropout_keep_prob=prob, seq_len=sub_seq_len)
        answer0_rnn_logits = text_rnn_model.build(a0_tensor, dropout_keep_prob=prob, seq_len=a0_seq_len)
        answer1_rnn_logits = text_rnn_model.build(a1_tensor, dropout_keep_prob=prob, seq_len=a1_seq_len)
        answer2_rnn_logits = text_rnn_model.build(a2_tensor, dropout_keep_prob=prob, seq_len=a2_seq_len)
        answer3_rnn_logits = text_rnn_model.build(a3_tensor, dropout_keep_prob=prob, seq_len=a3_seq_len)
        answer4_rnn_logits = text_rnn_model.build(a4_tensor, dropout_keep_prob=prob, seq_len=a4_seq_len)
    ...
    with tf.variable_scope("post_rnn", reuse=tf.AUTO_REUSE):
        ...
        subtitle_answer0_g_logits = tf.layers.dense(subtitle_answer0_g_rnn_logits, 1,
                                                    name="subtitle_answer_g_logits", kernel_regularizer=regularizer,
                                                    activation=tf.nn.leaky_relu)
        subtitle_answer1_g_logits = tf.layers.dense(subtitle_answer1_g_rnn_logits, 1,
                                                    name="subtitle_answer_g_logits", kernel_regularizer=regularizer, reuse=True,
                                                    activation=tf.nn.leaky_relu)
        ...
        '''
        subtitle_answer0_g_logits = tf.nn.dropout(subtitle_answer0_g_logits, prob[0][0])
        subtitle_answer1_g_logits = tf.nn.dropout(subtitle_answer1_g_logits, prob[0][0])
        subtitle_answer2_g_logits = tf.nn.dropout(subtitle_answer2_g_logits, prob[0][0])
        subtitle_answer3_g_logits = tf.nn.dropout(subtitle_answer3_g_logits, prob[0][0])
        subtitle_answer4_g_logits = tf.nn.dropout(subtitle_answer4_g_logits, prob[0][0])
    ...
    with tf.variable_scope("prediction"):
        ...
        cost = tf.reduce_mean(video_loss) + tf.cast(tf.contrib.layers.apply_regularization(regularizer, reg_losses),
                                                    tf.float32) + (
                       FLAGS.wd * tf.reduce_sum([tf.nn.l2_loss(v) for v in reg_var_list]))
        ...
        summary_op = tf.summary.merge([loss_summary, accuracy_summary])
        '''
    return cost, accuracy, video_accuracy, qid, comparison, train_var_list
