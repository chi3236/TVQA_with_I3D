...
sys.path.append("./tensorflow-glove")
#import tf_glove

srt_path = "./tvqa_subtitles"
srt_cache = "./sub_cache.cache"
qa_path = "."
frm_path = "/home/scw/Downloads/tvqa_new/frames_hq"
frm_cache = "./frm_cache.cache"
...
def load_srt(srt_dir, srt_cache_path):
    ...
    srt_paths = sorted(glob.glob(os.path.join(srt_dir, "*.srt")))
    name2sub_text = {}
    name2sub_time = {}
...
def add_srt(raw_data_dicts, srt_data, eos_token="<eos>"):
    ...
    for i in tqdm(range(len(data_dicts))):
        #if "Grey" in data_dicts[i]["show_name"]:
        if data_dicts[i]["vid_name"].startswith("s"):
            data_dicts[i]["vid_name"] = "bbt_" + data_dicts[i]["vid_name"]
        ...
        #grey_data_dicts.append(data_dicts[i])
    #return grey_data_dicts
    return data_dicts
...
def process_qa(qa_path, processed_srt, frame_base_path, frame_cnt_cache_path, save_path):
    ...
    for key in frame_cnt_dict:
        if key.startswith("s"):
            frame_cnt_dict["bbt_" + key] = frame_cnt_dict.pop(key)
    frame_cnt_dict[u'bbt_s06e12_seg02_clip_03'] = 191
    frame_cnt_dict[u'bbt_s06e12_seg02_clip_08'] = 164
    qa_srt_located_data = add_located(qa_srt_data, processed_srt, frame_cnt_dict)
    save_json(qa_srt_located_data, save_path)

if __name__ == '__main__':
    srt = load_srt(srt_path, srt_cache)
    srt = tokenize_srt(srt)
    raw_qa_files = glob.glob(os.path.join(qa_path, "tvqa_qa_release", "*jsonl"))

    for i, qa_file in enumerate(raw_qa_files):
        print("-" * 60)
        print("Processing %s" % qa_file)
        processed_qa_path = os.path.join(qa_path, os.path.split(qa_file)[1].replace(".jsonl", "_processed.json"))
        process_qa(qa_file, srt, frm_path, frm_cache, processed_qa_path)