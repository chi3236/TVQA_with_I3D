...
def tower_loss(scope, vocab_embedding, next_element):
    accuracy, qid, comparison = network_tower.build_graph(FLAGS, vocab_embedding,
...
def main(_):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            for j in range(FLAGS.num_gpus):
                ...
                with tf.device('/gpu:%d' % j):
                    with tf.name_scope('%s_%d' % (network_tower.TOWER_NAME, j)) as scope:
                        accuracy, qid, comparison = tower_loss(scope, vocab_embedding, next_element)
        ...
        for variable in tf.global_variables():
            if 'RGB' in variable.name:
                rgb_variable_map[variable.name.replace(':0', '')] = variable
        rgb_saver = tf.train.Saver(var_list=rgb_variable_map, reshape=True)

        flow_variable_map = {}
        for variable in tf.global_variables():
            if 'Flow' in variable.name or 'flow' in variable.name:
    ...
    with tf.Session(config=config) as sess:
        flow_saver.restore(sess, "./log/best_result/video3/flow_best/model-27450")
        rgb_saver.restore(sess, "./log/best_result/video3/rgb_best/model-25925")
        ...
        while True:
            try:
                #next_element = iterator.get_next()
                outputs = sess.run({'accuracy': accuracy_avg, 'qid': tower_qid, 'comparison': comparison_list})
                qids = np.concatenate((outputs['qid'][0], outputs['qid'][1])).tolist()
                comparisons = np.concatenate((outputs['comparison'][0], outputs['comparison'][1])).tolist()
                cummulative_acc += outputs["accuracy"]
                with open('log.txt', 'a') as f:
                    for index in range(len(comparisons)):
                        if not comparisons[index]:
                            f.write(str(qids[index][0]) + '\n')

                _global_step += 1
            except tf.errors.OutOfRangeError:
                break

    loss = cummulative_acc / _global_step
    print str(loss)
