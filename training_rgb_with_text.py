...
checkpoint_path = './log/checkpoint_rgb_and_text_full_train'
...
def tower_loss(scope, vocab_embedding, next_element):
    _, accuracy, video_accuracy, qid, comparison, train_var_list = network_tower_rgb_full.build_graph(FLAGS, vocab_embedding,
                                                                             next_element['rgb'],
    ...
    #for l in losses:
    loss_name = re.sub("%s_[0-9]*/" % network_tower_rgb_full.TOWER_NAME, '', _.op.name)
...
def main(_):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            for j in range(FLAGS.num_gpus):
                ...
                with tf.device('/gpu:%d' % j):
                    with tf.name_scope('%s_%d' % (network_tower_rgb_full.TOWER_NAME, j)) as scope:
        ...
        #    if variable.name.split('/')[0] == 'RGB' and not "Adam" in variable.name and not "rgb_fc" in variable.name:
        ...
        network_saver = tf.train.Saver(var_list, max_to_keep=100)
        ...
        with tf.Session(config=config) as sess:
            ...
            network_saver.restore(sess, './log/best_result/video3/rgb_best/model-25925')
            #rgb_saver.restore(sess, _CHECKPOINT_PATHS['rgb_imagenet'])
            #flow_saver.restore(sess, _CHECKPOINT_PATHS['flow_imagenet'])
            #text_saver.restore(sess, "./log/best_result/text1/model-37167")
            train_summary_writer = tf.summary.FileWriter('./log/train_rgb_and_text_full_train', sess.graph)
            val_summary_writer = tf.summary.FileWriter('./log/val_rgb_and_text_full_train', sess.graph)
