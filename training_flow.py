...
tf.flags.DEFINE_string("gpus", "2, 3", "GPUs to use")
...
tf.flags.DEFINE_float('lr_i3d', 1e-2, 'i3d_learn_rate')
...
checkpoint_path = './log/checkpoint_flow_and_text_no_se_lstm'
    ...
    def __call__(self):
        with hf.File(self.video_file, "r") as data:
            for qa in self.qa_file:
                ...
                return_value = {'of': of_images, 'sub': subtitles, 'q': question,
                                'a0': answer0, 'a1': answer1, 'a2': answer2, 'a3': answer3, 'a4': answer4,
                                'a': answer, 'qid': [qa['qid']], 'flow_seq_len': [len(of_images)], 'sub_seq_len': [len(subtitles)],
...
def tower_loss(scope, vocab_embedding, next_element):
    _, accuracy, video_accuracy, qid, comparison, train_i3d_var_list, train_match_var_list = network_tower_flow.build_graph(FLAGS, vocab_embedding,
                                                                             next_element['of'],
                                                                             ...
                                                                             next_element['qid'], next_element['flow_seq_len'],
    ...
    #for l in losses:
    loss_name = re.sub("%s_[0-9]*/" % network_tower_flow.TOWER_NAME, '', _.op.name)
...
def main(_):
    ...
    train_dataset = tf.data.Dataset.from_generator(train_generator, output_types={'of': tf.float32,
                                                                                  'sub': tf.int32, 'q': tf.int32,
                                                                                  'a0': tf.int32, 'a1': tf.int32,
                                                                                  'a2': tf.int32, 'a3': tf.int32,
                                                                                  'a4': tf.int32, 'a': tf.int32,
                                                                                  'qid': tf.int32, 'flow_seq_len': tf.int32,
    ...
    train_dataset = train_dataset.padded_batch(FLAGS.batch_size,
                                               padded_shapes={'of': [None, 224, 224, 2],
                                                              'sub': [None], 'q': [None], 'a0': [None], 'a1': [None],
                                                              'a2': [None], 'a3': [None], 'a4': [None], 'a': [5],
                                                              'qid': [1], 'flow_seq_len': [1], 'sub_seq_len': [1], 'q_seq_len': [1],
    ...
    val_dataset = tf.data.Dataset.from_generator(val_generator, output_types={'of': tf.float32,
                                                                              'sub': tf.int32, 'q': tf.int32,
                                                                              'a0': tf.int32, 'a1': tf.int32,
                                                                              'a2': tf.int32, 'a3': tf.int32,
                                                                              'a4': tf.int32, 'a': tf.int32,
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            for j in range(FLAGS.num_gpus):
                ...
                with tf.device('/gpu:%d' % j):
                    with tf.name_scope('%s_%d' % (network_tower_flow.TOWER_NAME, j)) as scope:
        ...
        flow_variable_map = {}
        for variable in tf.global_variables():
            if variable.name.split('/')[0] == 'Flow' and not "Momentum" in variable.name and not "Flow_fc" in variable.name and not "se3d" in variable.name:
        ...
        with tf.Session(config=config) as sess:
            ...
            #network_saver.restore(sess, "./log/checkpoint_flow_and_text_se/model-152540")
            #rgb_saver.restore(sess, _CHECKPOINT_PATHS['rgb_imagenet'])
            flow_saver.restore(sess, _CHECKPOINT_PATHS['flow_imagenet'])
            text_saver.restore(sess, "./log/best_result/text1/model-37167")
            train_summary_writer = tf.summary.FileWriter('./log/train_flow_and_text_no_se_lstm', sess.graph)
            val_summary_writer = tf.summary.FileWriter('./log/val_flow_and_text_no_se_lstm', sess.graph)
