...
checkpoint_path = './log/checkpoint_video_and_text'
...
def tower_loss(scope, vocab_embedding, next_element):
    _, accuracy, video_accuracy, qid, comparison, train_var_list = network_tower.build_graph(FLAGS, vocab_embedding,
    ...
    #for l in losses:
    loss_name = re.sub("%s_[0-9]*/" % network_tower.TOWER_NAME, '', _.op.name)
...
def main(_):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.Session(config=config) as sess:
            ...
            text_saver.restore(sess, "./log/best_result/1/model-37167")
            train_summary_writer = tf.summary.FileWriter('./log/train_video_and_text', sess.graph)
            val_summary_writer = tf.summary.FileWriter('./log/val_video_and_text', sess.graph)
