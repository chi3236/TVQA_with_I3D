...
class InceptionI3d(snt.AbstractModule):
  ...
  def se3d(self, net, name, output_dim):
    squeeze = tf.reduce_mean(net, axis=1)
    squeeze = tf.reduce_mean(squeeze, axis=1)
    squeeze = tf.reduce_mean(squeeze, axis=1)

    excitation = tf.layers.dense(squeeze, output_dim / 16, name=name + "_se3d_fc1")
    excitation = tf.nn.relu(excitation)
    excitation = tf.layers.dense(excitation, output_dim, name=name + "_se3d_fc2")
    excitation = tf.sigmoid(excitation)

    excitation = tf.reshape(excitation, [-1, 1, 1, 1, output_dim])
  ...
  def _build(self, inputs, is_training, dropout_keep_prob=1.0):
    ...
    with tf.variable_scope(end_point):
      ...
      #net = self.se3d(net, end_point, int(np.shape(net)[-1]))
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
