...
def build_graph(FLAGS, vocab_embedding, sub, q, a0, a1, a2, a3, a4, a, qid, sub_seq_len, q_seq_len, a0_seq_len, a1_seq_len, a2_seq_len, a3_seq_len, a4_seq_len, is_training):
    ...
    #with tf.device("/GPU:2"):
        ...
        with tf.variable_scope("Text_GRU", reuse=tf.AUTO_REUSE):
            ...
            sub_tensor = tf.nn.embedding_lookup(vocab_embedding, sub)
            q_tensor = tf.nn.embedding_lookup(vocab_embedding, q)
            a0_tensor = tf.nn.embedding_lookup(vocab_embedding, a0)
            a1_tensor = tf.nn.embedding_lookup(vocab_embedding, a1)
            a2_tensor = tf.nn.embedding_lookup(vocab_embedding, a2)
            a3_tensor = tf.nn.embedding_lookup(vocab_embedding, a3)
            a4_tensor = tf.nn.embedding_lookup(vocab_embedding, a4)
            ...
            question_rnn_logits = text_rnn_model.build(q_tensor, is_training=is_training, seq_len=q_seq_len)
            subtitle_rnn_logits = text_rnn_model.build(sub_tensor, is_training=is_training, seq_len=sub_seq_len)
            answer0_rnn_logits = text_rnn_model.build(a0_tensor, is_training=is_training, seq_len=a0_seq_len)
            answer1_rnn_logits = text_rnn_model.build(a1_tensor, is_training=is_training, seq_len=a1_seq_len)
            answer2_rnn_logits = text_rnn_model.build(a2_tensor, is_training=is_training, seq_len=a2_seq_len)
            answer3_rnn_logits = text_rnn_model.build(a3_tensor, is_training=is_training, seq_len=a3_seq_len)
            answer4_rnn_logits = text_rnn_model.build(a4_tensor, is_training=is_training, seq_len=a4_seq_len)
    ...
    #with tf.device("/GPU:1"):
        with tf.variable_scope("post_rnn", reuse=tf.AUTO_REUSE):
            ...
            if is_training is not None:
                subtitle_answer0_g_logits = tf.layers.dropout(subtitle_answer0_g_logits, training=True)
                subtitle_answer1_g_logits = tf.layers.dropout(subtitle_answer1_g_logits, training=True)
                subtitle_answer2_g_logits = tf.layers.dropout(subtitle_answer2_g_logits, training=True)
                subtitle_answer3_g_logits = tf.layers.dropout(subtitle_answer3_g_logits, training=True)
                subtitle_answer4_g_logits = tf.layers.dropout(subtitle_answer4_g_logits, training=True)
    ...
    with tf.device("/GPU:1"):
        with tf.variable_scope("prediction"):
            ...
            cost = tf.reduce_mean(loss) + tf.cast(tf.contrib.layers.apply_regularization(regularizer, reg_losses),
                                                  tf.float32) + (0.0001 * tf.reduce_sum([tf.nn.l2_loss(v) for v in reg_var_list]))
            ...
            summary_op = tf.summary.merge([loss_summary, accuracy_summary])
            '''
    return cost, accuracy, op, summary_op, qid, comparison
