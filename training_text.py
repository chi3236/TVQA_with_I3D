        ...
        with hf.File(self.video_file, "r") as data:
            ...
            for qa in self.qa_file:
                ...
                for word in qa['a0'].split():
                    embedding_temp += glove_embedding(word)
                answer_candidates.append(embedding_temp / len(qa['a0'].split()))

                embedding_temp = 0.0
                for word in qa['a1'].split():
                    embedding_temp += glove_embedding(word)
                answer_candidates.append(embedding_temp / len(qa['a1'].split()))

                embedding_temp = 0.0
                for word in qa['a2'].split():
                    embedding_temp += glove_embedding(word)
                answer_candidates.append(embedding_temp / len(qa['a2'].split()))

                embedding_temp = 0.0
                for word in qa['a3'].split():
                    embedding_temp += glove_embedding(word)
                answer_candidates.append(embedding_temp / len(qa['a3'].split()))

                embedding_temp = 0.0
                for word in qa['a4'].split():
                    embedding_temp += glove_embedding(word)
                answer_candidates.append(embedding_temp / len(qa['a4'].split()))

                answer = [0] * 5
                answer[qa['answer_idx']] = 1
                return_value = {'sub': subtitles, 'q': question,
                                'ac': answer_candidates, 'a': answer, 'sub_seq_len': [len(subtitles)],
...
def main(_):
    ...
    train_dataset = train_dataset.padded_batch(FLAGS.batch_size,
                                               padded_shapes={'sub': [None, 300], 'q': [None, 300], 'ac': [None, 300],
                                                              'a': [5], 'sub_seq_len': [1], 'q_seq_len': [1]})
