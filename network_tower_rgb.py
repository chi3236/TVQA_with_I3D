...
def build_graph(FLAGS, vocab_embedding, rgb_input, sub, q, a0, a1, a2, a3, a4, a, qid, rgb_seq_len, sub_seq_len,
    ...
    with tf.variable_scope("Flow"):
        flow_model = i3d.InceptionI3d(FLAGS.num_classes, spatial_squeeze=True, final_endpoint='Logits')
    ...
    # with tf.device("/GPU:1"):
    with tf.variable_scope("RGB_train_question_match", reuse=tf.AUTO_REUSE):
        rgb_logits = tf.layers.dense(rgb_logits, 400, name="RGB_text_fc", kernel_regularizer=regularizer,
                                     activation=tf.nn.leaky_relu)

        rgb_logits = tf.nn.dropout(rgb_logits, prob[0][0])
        rgb_logits = tf.layers.batch_normalization(rgb_logits, training=is_training[0][0])
        # question_d = tf.reduce_max(question_rnn_logits, axis=1, keepdims=True)
        question_rgb = tf.layers.dense(question_rnn_logits, 400, name="RGB_question_dense", kernel_regularizer=regularizer,
                                     activation=tf.nn.leaky_relu)
        question_rgb = tf.nn.dropout(question_rgb, prob[0][0])
        question_rgb = tf.layers.batch_normalization(question_rgb, training=is_training[0][0])
        rgb_question_mul = tf.matmul(rgb_logits, tf.transpose(question_rgb, perm=[0, 2, 1]))
        rgb_question_similarity = tf.nn.softmax(rgb_question_mul)
        rgb_question_masked = tf.matmul(rgb_question_similarity, question_rgb)
    ...
    with tf.variable_scope("RGB_train_answer_match", reuse=tf.AUTO_REUSE):
        ...
        answer0_rgb = tf.layers.dense(answer0_rnn_logits, 400, name="RGB_answer_dense", kernel_regularizer=regularizer,
                                     activation=tf.nn.leaky_relu)
        answer1_rgb = tf.layers.dense(answer1_rnn_logits, 400, name="RGB_answer_dense", kernel_regularizer=regularizer,
                                      activation=tf.nn.leaky_relu, reuse=True)
        answer2_rgb = tf.layers.dense(answer2_rnn_logits, 400, name="RGB_answer_dense", kernel_regularizer=regularizer,
                                      activation=tf.nn.leaky_relu, reuse=True)
        answer3_rgb = tf.layers.dense(answer3_rnn_logits, 400, name="RGB_answer_dense", kernel_regularizer=regularizer,
                                      activation=tf.nn.leaky_relu, reuse=True)
        answer4_rgb = tf.layers.dense(answer4_rnn_logits, 400, name="RGB_answer_dense", kernel_regularizer=regularizer,
                                      activation=tf.nn.leaky_relu, reuse=True)
        ...
        answer0_rgb = tf.layers.batch_normalization(answer0_rgb, training=is_training[0][0])
        answer1_rgb = tf.layers.batch_normalization(answer1_rgb, training=is_training[0][0])
        answer2_rgb = tf.layers.batch_normalization(answer2_rgb, training=is_training[0][0])
        answer3_rgb = tf.layers.batch_normalization(answer3_rgb, training=is_training[0][0])
        answer4_rgb = tf.layers.batch_normalization(answer4_rgb, training=is_training[0][0])

        rgb_answer0_mul = tf.matmul(rgb_logits, tf.transpose(answer0_rgb, perm=[0, 2, 1]))
        rgb_answer1_mul = tf.matmul(rgb_logits, tf.transpose(answer1_rgb, perm=[0, 2, 1]))
        rgb_answer2_mul = tf.matmul(rgb_logits, tf.transpose(answer2_rgb, perm=[0, 2, 1]))
        rgb_answer3_mul = tf.matmul(rgb_logits, tf.transpose(answer3_rgb, perm=[0, 2, 1]))
        rgb_answer4_mul = tf.matmul(rgb_logits, tf.transpose(answer4_rgb, perm=[0, 2, 1]))
        ...
        rgb_answer0_masked = tf.matmul(rgb_answer0_similarity, answer0_rgb)
        rgb_answer1_masked = tf.matmul(rgb_answer1_similarity, answer1_rgb)
        rgb_answer2_masked = tf.matmul(rgb_answer2_similarity, answer2_rgb)
        rgb_answer3_masked = tf.matmul(rgb_answer3_similarity, answer3_rgb)
        rgb_answer4_masked = tf.matmul(rgb_answer4_similarity, answer4_rgb)
    ...
    with tf.variable_scope("RGB_question_answer_att_layer", reuse=tf.AUTO_REUSE):
        rgb_question_answer0_att = tf.concat([rgb_logits, rgb_question_masked, rgb_answer0_masked,
                                              ...
                                              tf.multiply(rgb_logits, rgb_answer0_masked)], axis=2)
        rgb_question_answer1_att = tf.concat([rgb_logits, rgb_question_masked, rgb_answer1_masked,
                                              tf.multiply(rgb_logits, rgb_question_masked),
                                              tf.multiply(rgb_logits, rgb_answer1_masked)], axis=2)
        rgb_question_answer2_att = tf.concat([rgb_logits, rgb_question_masked, rgb_answer2_masked,
                                              tf.multiply(rgb_logits, rgb_question_masked),
                                              tf.multiply(rgb_logits, rgb_answer2_masked)], axis=2)
        rgb_question_answer3_att = tf.concat([rgb_logits, rgb_question_masked, rgb_answer3_masked,
                                              tf.multiply(rgb_logits, rgb_question_masked),
                                              tf.multiply(rgb_logits, rgb_answer3_masked)], axis=2)
        rgb_question_answer4_att = tf.concat([rgb_logits, rgb_question_masked, rgb_answer4_masked,
                                              tf.multiply(rgb_logits, rgb_question_masked),
                                              tf.multiply(rgb_logits, rgb_answer4_masked)], axis=2)

        #rgb_rnn = video_rnn.vRNN('GRU', 1000, 'RGB')

        #rgb_question_answer0_att = rgb_rnn.build(rgb_question_answer0_att, prob, rgb_seq_len)
        #rgb_question_answer1_att = rgb_rnn.build(rgb_question_answer1_att, prob, rgb_seq_len)
        #rgb_question_answer2_att = rgb_rnn.build(rgb_question_answer2_att, prob, rgb_seq_len)
        #rgb_question_answer3_att = rgb_rnn.build(rgb_question_answer3_att, prob, rgb_seq_len)
        #rgb_question_answer4_att = rgb_rnn.build(rgb_question_answer4_att, prob, rgb_seq_len)

        #rgb_question_answer0_att = tf.reduce_max(rgb_question_answer0_att, axis=1)
        #rgb_question_answer1_att = tf.reduce_max(rgb_question_answer1_att, axis=1)
        #rgb_question_answer2_att = tf.reduce_max(rgb_question_answer2_att, axis=1)
        #rgb_question_answer3_att = tf.reduce_max(rgb_question_answer3_att, axis=1)
        #rgb_question_answer4_att = tf.reduce_max(rgb_question_answer4_att, axis=1)
        ...
        '''
        '''
        rgb_question_answer0_fc = tf.layers.dense(rgb_question_answer0_att, 2000, activation=tf.nn.leaky_relu,
                                                  kernel_regularizer=regularizer, name='rgb_mask_fc')
        rgb_question_answer1_fc = tf.layers.dense(rgb_question_answer1_att, 2000, activation=tf.nn.leaky_relu,
                                                  kernel_regularizer=regularizer, name='rgb_mask_fc', reuse=True)
        rgb_question_answer2_fc = tf.layers.dense(rgb_question_answer2_att, 2000, activation=tf.nn.leaky_relu,
                                                  kernel_regularizer=regularizer, name='rgb_mask_fc', reuse=True)
        rgb_question_answer3_fc = tf.layers.dense(rgb_question_answer3_att, 2000, activation=tf.nn.leaky_relu,
                                                  kernel_regularizer=regularizer, name='rgb_mask_fc', reuse=True)
        rgb_question_answer4_fc = tf.layers.dense(rgb_question_answer4_att, 2000, activation=tf.nn.leaky_relu,
                                                  kernel_regularizer=regularizer, name='rgb_mask_fc', reuse=True)
        ...
        '''
        #rgb_question_answer0_fc = tf.reduce_max(rgb_question_answer0_att, 1)
        #rgb_question_answer1_fc = tf.reduce_max(rgb_question_answer1_att, 1)
        #rgb_question_answer2_fc = tf.reduce_max(rgb_question_answer2_att, 1)
        #rgb_question_answer3_fc = tf.reduce_max(rgb_question_answer3_att, 1)
        #rgb_question_answer4_fc = tf.reduce_max(rgb_question_answer4_att, 1)
    '''
    with tf.variable_scope("Flow_question_match", reuse=tf.AUTO_REUSE):
        ...
        flow_question_mul = tf.matmul(tf.transpose(question_d, [0, 2, 1]), flow_logits)
    ...
    with tf.variable_scope("Flow_question_answer_att_layer", reuse=tf.AUTO_REUSE):
        ...
        flow_question_answer0_fc = tf.layers.dense(flow_question_answer0_att, 1, activation=tf.nn.leaky_relu,
                                                   kernel_regularizer=regularizer, name="flow_mask_fc")
        flow_question_answer1_fc = tf.layers.dense(flow_question_answer1_att, 1, activation=tf.nn.leaky_relu,
                                                   kernel_regularizer=regularizer, name="flow_mask_fc", reuse=True)
        flow_question_answer2_fc = tf.layers.dense(flow_question_answer2_att, 1, activation=tf.nn.leaky_relu,
                                                   kernel_regularizer=regularizer, name="flow_mask_fc", reuse=True)
        flow_question_answer3_fc = tf.layers.dense(flow_question_answer3_att, 1, activation=tf.nn.leaky_relu,
                                                   kernel_regularizer=regularizer, name="flow_mask_fc", reuse=True)
        flow_question_answer4_fc = tf.layers.dense(flow_question_answer4_att, 1, activation=tf.nn.leaky_relu,
                                                   kernel_regularizer=regularizer, name="flow_mask_fc", reuse=True)
    ...
    with tf.variable_scope("video_fc", reuse=tf.AUTO_REUSE):
        rgb_answer0_g_logits = tf.layers.dense(rgb_question_answer0_att, 1, name="RGB_fc",
                                               kernel_regularizer=regularizer, activation=tf.nn.leaky_relu)
        rgb_answer1_g_logits = tf.layers.dense(rgb_question_answer1_att, 1, name="RGB_fc",
                                               kernel_regularizer=regularizer, activation=tf.nn.leaky_relu, reuse=True)
        rgb_answer2_g_logits = tf.layers.dense(rgb_question_answer2_att, 1, name="RGB_fc",
                                               kernel_regularizer=regularizer, activation=tf.nn.leaky_relu, reuse=True)
        rgb_answer3_g_logits = tf.layers.dense(rgb_question_answer3_att, 1, name="RGB_fc",
                                               kernel_regularizer=regularizer, activation=tf.nn.leaky_relu, reuse=True)
        rgb_answer4_g_logits = tf.layers.dense(rgb_question_answer4_att, 1, name="RGB_fc",
                                               kernel_regularizer=regularizer, activation=tf.nn.leaky_relu, reuse=True)
        ...
        rgb_answer0_g_logits = tf.layers.batch_normalization(rgb_answer0_g_logits, training=is_training[0][0])
        rgb_answer1_g_logits = tf.layers.batch_normalization(rgb_answer1_g_logits, training=is_training[0][0])
        rgb_answer2_g_logits = tf.layers.batch_normalization(rgb_answer2_g_logits, training=is_training[0][0])
        rgb_answer3_g_logits = tf.layers.batch_normalization(rgb_answer3_g_logits, training=is_training[0][0])
        rgb_answer4_g_logits = tf.layers.batch_normalization(rgb_answer4_g_logits, training=is_training[0][0])

        rgb_answer0_g_logits = tf.reduce_max(rgb_answer0_g_logits, 1)
        rgb_answer1_g_logits = tf.reduce_max(rgb_answer1_g_logits, 1)
        rgb_answer2_g_logits = tf.reduce_max(rgb_answer2_g_logits, 1)
        rgb_answer3_g_logits = tf.reduce_max(rgb_answer3_g_logits, 1)
        rgb_answer4_g_logits = tf.reduce_max(rgb_answer4_g_logits, 1)
        '''
    ...
    with tf.variable_scope("prediction"):
        ...
        def comparison_fn(text, rgb, flow):
            if text == rgb:
                result = text
            elif text == flow:
                result = text
                ...
                result = rgb
            elif text != rgb and rgb != flow and text != flow:
                result = text
        ...
        def voting(elem):
            ...
            rgb_label = elem[1]
            flow_label = elem[2]
            comparison = tf.contrib.eager.py_func(func=comparison_fn, inp=[text_label, rgb_label, flow_label], Tout=tf.int64)
            return comparison

        # video_logits = rgb_answer_embed + flow_answer_embed
        video_logits = rgb_answer_embed
        ...
        for v in tf.trainable_variables():
            if v.name.startswith(u"RGB_train") or v.name.startswith(u"video") or v.name.startswith(u"RGB_question_answer"):
                train_match_var_list.append(v)
            if v.name.startswith(u"RGB") and not u"train" in v.name:
                train_i3d_var_list.append(v)
            #if not 'bias' in v.name and 'bidirectional_rnn' in v.name and not 'post' in v.name and not 'Text' in v.name:
